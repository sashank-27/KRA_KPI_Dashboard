...
interface UserManagementProps {
  ...
  onCreateUser: (user: Omit<User, "_id">) => void;
...
export function UserManagement({
      ...
      try {
        const res = await fetch(`${getApiBaseUrl()}/api/users`, {
          ...
          body: JSON.stringify({
            ...
            department:
              typeof newUser.department === "string"
                ? newUser.department
                : (newUser.department as Department)?._id,
            role: newUser.role,
  ...
  const handleUpdateUserLocal = async (updatedUser: User) => {
    try {
      ...
      setUsers(users.map((u) => (u._id === updatedUser._id ? updatedUserResponse : u)));
    } catch (err) {
